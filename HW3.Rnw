...
<<tidy=TRUE, fig.width=8, fig.height=6, out.width="8in", out.height="6in">>==
  ...
  ggplot(drvdat, aes(x=reorder(drv, value, FUN=mean), y=value))+geom_boxplot()+geom_jitter(alpha=.5)+facet_wrap("variable")+ylab("MPG")+xlab("Drive")+ggtitle("Drive Influences both Highway and City MPG Rating\n")
@
\end{document}