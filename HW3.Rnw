...
@
\newpage
\subsection*{Code Block 1: Distribution of MPG Ratings}
<<tidy=TRUE>>==
  ...
  d = ggplot(displdata, aes(x=displ, y=value, color=variable))+geom_point()+geom_smooth(method="lm")+xlab("Engine Displacement")+ylab("MPG")+ggtitle("Engine Displacement is Directly Correlated to MPG Rating")
  e =ggplot(mpg, aes(x=cyl, y=displ))+geom_boxplot()+xlab("# of Cylinders")+ylab("Engine Displacement")+ggtitle("Engine Displacement is Directly Correlated to MPG Rating")
...
@
In Figure 1, I began to examine the distribution of the two MPG variables (City and Highway).We may note from the first plot (a) in Figure 1, that there is a fairly strong, direct linear correlation between highway and city mileage for any given car. In all cases, the highway mileage was higher than city mileage. Indeed, we see confirmed in the second plot (b) of Figure 1, that on average highway is greater than city MPG ratings. In the third plot (c) of the figure, I wanted to continue my investigation of trends in MPG rating; I chose to examine the question of whether there were significant changes in average MPG rating over the course of the ten year period of study. Indeed, both plotting and t-testing revealed no overall improvement in MPG rating from 1999 to 2008. Finding no explanatory trend there, I decided to begin investigating the role of engine characteristics in MPG rating. In the final two plots (d & e)of this figure, I demonstrate the relationship direct, linear relationship between engine displacement and MPG rating; followed by the relationship between number of cylinders and displacement. This final plot leads us nicely into our subsequent figures.
\newpage
\subsection*{Code Block 2: Which Manufacturers Have the Best Averge MPG Rating?}
<<tidy=TRUE>>==
    catplotdat = melt(mpg, measure.vars=c("hwy", "cty"), id.vars =c("id", "manufacturer", "class", "cyl", "year"))
      f = ggplot(catplotdat, aes(x=reorder(manufacturer, value, FUN=mean), y=value))+geom_boxplot()+geom_jitter(size=1)+facet_grid(variable~.)+theme(axis.text.x = element_text(angle=90))+xlab("Manufacturer")+ggtitle("MPG Ratings By Manufacturer\n")
@
In figure two, we wanted to see which manufacturers produced the highest efficiency cars on average. Overall, the average Honda car had the best MPG rating for both city and highway, trailed by Volkswagen. Notably, the most efficient cars in this dataset were manufactured by Volkswagen. On the other side, manufacturers like Land Rover and Lincoln had the worst performance.}
\subsection*{Code Block 3: Relationship between Cylinders and MPG}
<<>>=
...
<<echo=FALSE, fig.width=8, fig.height=9, out.width="8in", out.height="9in", warning=FALSE>>==
    grid.newpage()
...
<<echo=FALSE, fig.width=12, fig.height=13.5, out.width="8in", out.height="9in", warning=FALSE>>==
    grid.newpage()
...
<<tidy=TRUE, fig.width=8, fig.height=6, out.width="8in", out.height="6in">>==
  ...
  ggplot(drvdat, aes(x=reorder(drv, value, FUN=mean), y=value))+geom_boxplot()+geom_jitter(alpha=.5)+geom_smooth(method="lm", color="red")+facet_wrap("variable")+ylab("MPG")+xlab("Drive")
@
\end{document}